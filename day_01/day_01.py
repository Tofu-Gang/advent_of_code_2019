...
def puzzle_1() -> None:
    ...
    The answer should be 3235550.
    """

    with open("day_01/input.txt", 'r') as f:
        masses = [int(line.strip()) for line in f.readlines()]
        print(sum([_get_fuel_requirement(mass) for mass in masses]))
...
def puzzle_2() -> None:
    ...
    with open("day_01/input.txt", 'r') as f:
        ...
        for mass in masses:
            fuel = _get_fuel_requirement(mass)
            fuel_count += fuel

            # now treat the fuel as a module with mass as well and count the
            # fuel needed to launch the fuel...to launch the module
            while True:
                mass = fuel
                fuel = _get_fuel_requirement(mass)
